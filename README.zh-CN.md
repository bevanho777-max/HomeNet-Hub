...
环境变量的唯一理由。删行同时会让所有会话失效,因为签名密钥跟着那一行一起没了。
...
| `ADMIN_PASSWORD` | 管理端点(发现、整个凭据 API、以及对运行时目标的*写入*)的**引导**密码(8–256 字符;`openssl rand -base64 24`)。只用一次,在还没有 `admin_auth` 行的安装上创建那一行(哈希存入);此后以数据库为准,这个变量不再起作用。不设**且**无该行 → 这些端点一律 **401**,而不是"放开"。删掉那一行即可让它重新引导 —— 这就是忘记密码时的恢复手段。 |
