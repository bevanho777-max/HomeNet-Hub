...
- **中 / EN 语言切换** — header 里一个小开关,存 `localStorage`,纯浏览器本地偏好:不进服务端
  状态,也不影响别人看到的界面。面板自身的界面文案内置两套;YAML 里的展示文本给它加一个
  `_en` 兄弟字段(`title_en` / `hint_en` / `name_en` / `columns_en` …)就跟着切,没加的字段
  两种语言都照原值渲染。
